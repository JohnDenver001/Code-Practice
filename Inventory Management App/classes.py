...
class Inventory:
    ...
    def save_inventory_to_json(self):
        """Save the inventory of the current_user"""
    ...
    def add_product(self):
        ...
        self.current_user_inventory.append(product)
    ...
    def remove_product(self):
        ...
        for product in self.current_user_inventory:
            if product.product_id == search_id:
                self.current_user_inventory.remove(product)
...
class User():
    ...
    def load_user_credentials_from_json(self):
        ...
        try:
            ...
            for user in user_data:
                user_credentials = {"username": user["username"], "password": user["password"].encode()}
    ...
    def save_user(self, new_registered_user):
        ...
        self.inventory.user_inventory_list.append(new_registered_user)

        with open(inventory_database_path, "w") as file:
            json.dump(self.inventory.user_inventory_list, file, indent = 4)
            
    def load_users_inventory(self):
        """Load all users inventory and add it to self.inventory.user_inventory_list"""
    ...
    def register(self):
        ...
        salt = bcrypt.gensalt() #Generate a random data
    ...
    def login(self):
        ...
        if not bcrypt.checkpw(password.encode(), current_user["password"]):
